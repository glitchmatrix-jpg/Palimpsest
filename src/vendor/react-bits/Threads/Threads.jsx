...
export default function Threads({color=[.84,.7,.36],amplitude=.65,distance=.15,enableMouseInteraction=true,className=''}) {
 ...
 useEffect(()=>{const c=ref.current;if(!c)return;const renderer=new Renderer({alpha:true});const gl=renderer.gl;gl.clearColor(0,0,0,0);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);c.appendChild(gl.canvas);
 ...
 return()=>{cancelAnimationFrame(rafRef.current);ro.disconnect();io.disconnect();c.removeEventListener('mousemove',mm);c.removeEventListener('mouseleave',ml);if(c.contains(gl.canvas))c.removeChild(gl.canvas);gl.getExtension('WEBGL_lose_context')?.loseContext();};},[color,amplitude,distance,enableMouseInteraction]);
